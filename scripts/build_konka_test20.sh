...
grep -Fq 'private int security = 1;' "$STUDIO/freeRDPCore/src/main/java/com/freerdp/freerdpcore/domain/BookmarkBase.java"
grep -Fq 'private int tlsSecLevel = 0;' "$STUDIO/freeRDPCore/src/main/java/com/freerdp/freerdpcore/domain/BookmarkBase.java"
grep -Fq 'sharedPrefs.getInt(keySecurity, 1)' "$STUDIO/freeRDPCore/src/main/java/com/freerdp/freerdpcore/domain/BookmarkBase.java"
grep -Fq 'sharedPrefs.getInt(keyTlsSecLevel, 0)' "$STUDIO/freeRDPCore/src/main/java/com/freerdp/freerdpcore/domain/BookmarkBase.java"
...
Base: proven Test19 Android 9/API28 runtime
ABI: armeabi-v7a only
minSdk=28; targetSdk=28
Native platform: android-28 (NDK r25c)
Android 9 API29 backports retained
Android NewApi lint gate: 0 findings required
Bookmark DB remains standard Room/SQLite; no destructive DB schema change
New profile default security: RDP (/sec:rdp)
New profile default TLS security level: 0 (/tls:seclevel:0, allow legacy algorithms)
TLS minimum version remains default/unforced
